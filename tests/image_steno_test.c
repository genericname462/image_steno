...
int consistency_test() {
    ...
    if (embed_data(image, 1024, 1024, 4, (uint8_t*) &data, data_size) != 0) {
        result = -2;
        goto fail;
    }

    if (extract_data(image, 1024, 1024, 4, output_data, data_size) != 0) {
        result = -3;
        goto fail;
    }
...
int get_bit_test() {
    ...
    for (unsigned int i = 0; i < sizeof(test_bytes_get_bit) / sizeof(test_bytes_get_bit[0]); ++i) {
        ...
        if (bit != test_bytes_get_bit[i][2]) {
            printf("%i[%i] is %i, expected %i\n", test_bytes_get_bit[i][0], test_bytes_get_bit[i][1], bit, test_bytes_get_bit[i][2]);
...
int set_bit_test() {
    ...
    for (unsigned int i = 0; i < sizeof(test_bytes_set_bit) / sizeof(test_bytes_set_bit[0]); ++i) {
        ...
        if (test_bytes_set_bit[i][0] != test_bytes_set_bit[i][3]) {
            printf("Testcase %i: Got %i after setting bit %i at %i, but expected %i\n", i+1, test_bytes_set_bit[i][0], test_bytes_set_bit[i][2], test_bytes_set_bit[i][1], test_bytes_set_bit[i][3]);
